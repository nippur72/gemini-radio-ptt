...
La volante ha fatto un sacco di multe per divieto di sosta.`;
  ...
  function updateLedMeter(meterId: string, percent: number) {
  ...
  radio.onRxVolumeChange = (percent: number) => {
    updateLedMeter('rx-led-meter', percent);
  };

  // Update UI states for mobile interface
  function updateUI() {
    if (!statusLed || !statusText || !glowRing || !pttBtn) return;
  ...
  function handlePressEnd(e: Event) {
    e.preventDefault();
    if (!isPointerDown) return;
    isPointerDown = false;
  ...
  function handlePointerCancelOrLeave(e: Event) {
    if (!isPointerDown) return;
    isPointerDown = false;
