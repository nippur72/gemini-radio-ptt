...
class RadioApp {
    ...
    FALLBACK_FILTER_MIN_FREQ = 300;
    FALLBACK_FILTER_MAX_FREQ = 3000;
    DISTORTION_AMOUNT = 3.5;
    ...
    async initAudio() {
        ...
        this.distortionNode.curve = makeDistortionCurve(this.DISTORTION_AMOUNT);
    ...
    fadeNoise(targetVolume) {
        if (!this.audioCtx || !this.noiseGainNode)
            return;
        ...
        if (Math.abs(currentVal - targetVolume) < 0.001)
            return;
    ...
    // Play Squelch Tail
    playSquelchTail() {
        if (!this.audioCtx || !this.dspConfig.squelchTailEnabled)
            return;
        const sampleRate = this.audioCtx.sampleRate;
        const duration = 0.25;
    ...
    async initWebSocket(apiKey, model, voice, systemInstruction) {
        ...
        try {
            ...
            this.ws.onclose = (event) => {
                ...
                setTimeout(() => {
                    if (this.wsConfig) {
                        this.initWebSocket(this.wsConfig.apiKey, this.wsConfig.model, this.wsConfig.voice, this.wsConfig.systemInstruction);
                    }
                }, 4000);
            };
            this.ws.onerror = (err) => {
    ...
    async initMicStream() {
        ...
        try {
            ...
            this.micLoopbackGainNode.gain.setValueAtTime(this.dspConfig.micLoopbackEnabled ? 1.0 : 0.0, this.audioCtx.currentTime);
